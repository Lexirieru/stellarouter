...
export default function TransitionScribble() {
    useEffect(() => {
        const logoStellarouterClickable = document.querySelector('.logo-stellarouter');
        ...
        if (!logoStellarouterClickable || !transitionScribblePath || !transitionScribbleSvg) return;
        ...
        const runScribbleAnimation = (e) => {
            ...
            if (!transitionLogo) {
                ...
                const svgClone = document.querySelector('.logo-stellarouter').cloneNode(true);
        ...
        logoStellarouterClickable.addEventListener('click', runScribbleAnimation);

        // Auto-run on load
        const timer = setTimeout(() => runScribbleAnimation(null), 100);

        return () => {
            logoStellarouterClickable.removeEventListener('click', runScribbleAnimation);
            clearTimeout(timer);
        };
    }, []);
